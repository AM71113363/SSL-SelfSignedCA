...
int CreateSelfsignedCert( DATA *input)
{
    ...
    if(ret != 0 ){
        ErrorMsgHandle("ERROR: mbedtls_ctr_drbg_seed",ret);
      return ret;
    }

    ret = mbedtls_mpi_read_string( &serial, 10, input->SerialNumber);
    if(ret != 0 ){
        ErrorMsgHandle("ERROR: mbedtls_mpi_read_string",ret);
      return ret;
    }
    ...
    if(ret != 0 ){
        ErrorMsgHandle("ERROR: mbedtls_x509write_crt_set_subject_name",ret);
      return ret;
    }

    ret = mbedtls_x509write_crt_set_issuer_name( &NewCert, input->IssuerName );
    if(ret != 0 ){
        ErrorMsgHandle("ERROR: mbedtls_x509write_crt_set_issuer_name",ret);
      return ret;
    }
    ...
    if(ret != 0 ){
        ErrorMsgHandle("ERROR: mbedtls_x509write_crt_set_serial",ret);
        return ret;
    }

    ret = mbedtls_x509write_crt_set_validity( &NewCert, NOT_BEFORE, NOT_AFTER );
    if(ret != 0 ){
        ErrorMsgHandle("ERROR: mbedtls_x509write_crt_set_validity",ret);
      return ret;
    }

    ret = mbedtls_x509write_crt_set_basic_constraints( &NewCert, 1, input->max_pathlen );
    if(ret != 0 ){
        ErrorMsgHandle("ERROR: mbedtls_x509write_crt_set_basic_constraints",ret);
      return ret;
    }

    ret = mbedtls_x509write_crt_set_subject_key_identifier( &NewCert );
    if(ret != 0 ){
        ErrorMsgHandle("ERROR: mbedtls_x509write_crt_set_subject_key_identifier",ret);
      return ret;
    }

    ret = mbedtls_x509write_crt_set_authority_key_identifier( &NewCert );
    if(ret != 0 ){
        ErrorMsgHandle("ERROR: mbedtls_x509write_crt_set_authority_key_identifier",ret);
      return ret;
    }
    ...
    if( input->KeyUsage != 0 )
    {
        ...
        if(ret != 0 ){
            ErrorMsgHandle("ERROR: mbedtls_x509write_crt_set_key_usage",ret);
          return ret;
        }
  ...
  return write_certificate( &NewCert, input->CertName, &ctr_drbg);
}
